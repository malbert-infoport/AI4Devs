# TASK-004-FE: Implementar grid de organizaciones con Kendo UI y columnas calculadas

=============================================================
**TICKET ID:** TASK-004-FE  
...
- **Filtros**: OData query parameters para filtrado server-side
...
export class OrganizationService {
  ...
  getAll(params: {
    ...
    search?: string;
    groupId?: number;
  }): Observable<{ data: OrganizationListItem[]; total: number }> {
    let queryParams = new HttpParams()
      .set('$skip', params.skip.toString())
      .set('$top', params.take.toString());

    // Filtro por estado
    if (params.status === 'active') {
      queryParams = queryParams.set('$filter', 'AuditDeletionDate eq null');
    } else if (params.status === 'inactive') {
      queryParams = queryParams.set('$filter', 'AuditDeletionDate ne null');
    }

    // Búsqueda
    if (params.search) {
      const searchFilter = `contains(Name, '${params.search}') or contains(Cif, '${params.search}')`;
      queryParams = queryParams.set('$filter', searchFilter);
    }

    // Filtro por grupo
    if (params.groupId) {
      queryParams = queryParams.set('$filter', `GroupId eq ${params.groupId}`);
    }

    return this.http.get<{ data: OrganizationListItem[]; total: number }>(
      this.apiUrl,
      { params: queryParams }
    );
  }

...
function Fix-EntityModelNamespace {
    ...
    try {
        ...
        # Reemplazar namespace incorrecto por el correcto (soporta ; o {)
        $pattern = '\bnamespace\s+' + [regex]::Escape("$ProjectName.Back.DataModel")
        $replacement = 'namespace ' + "$ProjectName.Back.Data.DataModel"

        if ([regex]::IsMatch($content, $pattern)) {
            $newContent = [regex]::Replace($content, $pattern, $replacement, [System.Text.RegularExpressions.RegexOptions]::None)
            if ($newContent -ne $content) {
                Set-Content -Path $entityModelPath -Value $newContent -NoNewline
                Write-Success "Namespace corregido: $replacement"
            } else {
                Write-Host "  El namespace ya es correcto" -ForegroundColor $script:InfoColor
            }
            return $true
        } else {
...
function Move-EntityClasses {
    ...
    foreach ($file in $filesToMove) {
        try {
            ...
            # Post-procesar el archivo movido: asegurar que las propiedades ICollection no sean solo lectura
            try {
                $destContent = Get-Content -Path $destPath -Raw
                $origDestContent = $destContent

                # Reemplazar propiedades get-only de ICollection por get; set; (acepta System.Collections.Generic.ICollection)
                $pattern = '(\bpublic\s+virtual\s+(?:[\w\.]+\.)?ICollection<[^>]+>\s+\w+\s*)\{\s*get;\s*\}'
                $replacement = '$1{ get; set; }'
                $newDestContent = [regex]::Replace($destContent, $pattern, $replacement)

                if ($newDestContent -ne $origDestContent) {
                    Set-Content -Path $destPath -Value $newDestContent -Encoding UTF8 -NoNewline
                    Write-Host "    [FIX] Converted ICollection properties to read-write in $($file.Name)" -ForegroundColor $script:InfoColor
                }
            } catch {
                Write-Warning-Message "No se pudo post-procesar $($file.Name): $_"
            }
...
function Fix-NetStandardCompatibility {
    ...
    foreach ($file in $entityFiles) {
        try {
            ...
            # 7. Convertir propiedades ICollection de solo lectura a read-write
            $collPattern = '(\bpublic\s+virtual\s+(?:[\w\.]+\.)?ICollection<[^>]+>\s+\w+\s*)\{\s*get;\s*\}'
            if ([regex]::IsMatch($content, $collPattern)) {
                $newContentColl = [regex]::Replace($content, $collPattern, '$1{ get; set; }')
                if ($newContentColl -ne $content) {
                    $content = $newContentColl
                    $fileFixes++
                }
            }

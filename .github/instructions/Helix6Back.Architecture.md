...
Cuando generes o modifiques código C# en este proyecto, debes:

1. **Seguir estrictamente las convenciones de nomenclatura** documentadas en la sección 2.2
2. **Respetar la estructura de capas** definida en la arquitectura (Api → Services → Data → DataModel)
3. **Heredar de las clases base** apropiadas (`BaseService`, `BaseRepository`, `IEntityBase`)
4. **Implementar los hooks del ciclo de vida** cuando sea necesario (`ValidateView`, `PreviousActions`, `PostActions`)
5. **Usar los tipos correctos** (`View` para DTOs, `Entity` para modelos de BD)
6. **Aplicar auditoría automática** en todas las entidades (campos `Audit*`)
7. **Habilitar Nullable Reference Types** (`#nullable enable`)
8. **Organizar imports** (System → Third-party → Project, alfabéticamente)
9. **Campos de auditoría de fecha siempre nullable:** `AuditCreationDate` y `AuditModificationDate` deben declararse como `DateTime?` en C# (para cumplir `IEntityFramework`) y como `TIMESTAMPTZ` **sin** `NOT NULL` ni `DEFAULT` en scripts SQL. El framework Helix6 los rellena automáticamente en el ciclo de vida del repositorio; si se declaran `NOT NULL DEFAULT CURRENT_TIMESTAMP` en BD, el scaffolding inverso los genera como `DateTime` (non-nullable) rompiendo la interfaz `IEntityFramework`.
10. **Clave primaria siempre `int` / `SERIAL`:** La interfaz `IEntityFramework` declara `int Id { get; set; }`. Por tanto, **todas** las tablas deben definir su clave primaria como `SERIAL PRIMARY KEY` (mapea a `int` en C#). Usar `BIGSERIAL` (→ `long`) o cualquier otro tipo rompe el contrato de la interfaz y produce errores CS0738 al compilar el DataModel. Esto aplica también a tablas de auditoría como `AuditLog`.
...
## 2. Estructura de Proyecto Helix6

### 2.1 Estructura de Carpetas Estándar

Todo proyecto basado en Helix6 sigue una estructura de carpetas estandarizada que facilita la navegación, mantenimiento y escalabilidad. A continuación se detalla la estructura completa de un proyecto típico:

```
[NombreProyecto]/
├── .dockerignore
├── .gitignore
├── .template.config/               # Configuración de plantilla para dotnet new
├── Directory.Build.props            # Propiedades compartidas MSBuild
├── [Proyecto].sln                   # Solución principal
├── README.md                        # Documentación del proyecto
├── publish-template.yml             # Plantilla de publicación Azure DevOps
│
├── docs/                            # Documentación técnica
│   └── [Proyecto]_Architecture.md   # Documento de arquitectura
│
├── [Proyecto].Api/                  # Capa de Presentación (Web API)
│   ├── Program.cs                   # Punto de entrada de la aplicación
│   ├── ApiConsts.cs                 # Constantes de la API
│   ├── HelixEntities.xml            # Configuración del generador
│   ├── Dockerfile                   # Containerización
│   ├── web.config                   # Configuración IIS
│   ├── appsettings.json             # Configuración principal
│   ├── appsettings.Development.json # Configuración desarrollo
│   ├── appsettings.CI.json          # Configuración integración continua
│   │
│   ├── Endpoints/                   # Definición de endpoints
│   │   ├── Endpoints.cs             # Endpoints personalizados
│   │   └── Base/                    # Endpoints base y generados
│   │       ├── GenericEndpoints.cs  # Registro de endpoints generados
│   │       ├── AttachmentEndpoints.cs
│   │       ├── SecurityEndpoints.cs
│   │       └── Generator/           # Endpoints auto-generados
│   │           ├── WorkerEndpoints.cs
│   │           ├── CourseEndpoints.cs
│   │           └── [Entidad]Endpoints.cs
│   │
│   ├── Extensions/                  # Métodos de extensión y configuración
│   │   ├── DependencyInjection.cs   # Configuración de DI
│   │   ├── AuthConfiguration.cs     # Configuración de autenticación
│   │   ├── CorsConfiguration.cs     # Configuración de CORS
│   │   ├── CultureConfiguration.cs  # Configuración de i18n
│   │   ├── MapsterConfiguration.cs  # Configuración de mapeos
│   │   └── SwaggerConfiguration.cs  # Configuración de OpenAPI
│   │
│   ├── Security/                    # Implementaciones de seguridad
│   │   ├── KeyCloakUserClaimsMapping.cs
│   │   ├── APVClaimsMapping.cs
│   │   ├── APVReferenceTokenValidation.cs
│   │   └── UserContext.cs           # Contexto del usuario actual
│   │
│   ├── Attachments/                 # Implementaciones de almacenamiento
│   │   ├── AttachmentDBSource.cs
│   │   └── AttachmentDriveSource.cs
│   │
│   ├── Resources/                   # Recursos multiidioma
...
│
├── [Proyecto].DataModel/            # Capa de Modelo de Datos
│   ├── [Entidad].cs                 # Entidades del dominio
│   │   ├── Worker.cs
│   │   ├── Course.cs
│   │   └── Project.cs
│   │
│   └── Base/                        # Entidades base del framework
│       ├── Attachment.cs
│       ├── AttachmentFile.cs
│       ├── SecurityUser.cs
│       ├── SecurityProfile.cs
...
│       └── Permissions.cs
│
├── [Proyecto].Entities/             # Capa de DTOs/Views
│   ├── Views/                       # Clases View generadas
│   │   ├── WorkerView.cs
│   │   ├── CourseView.cs
│   │   ├── ProjectView.cs
│   │   │
│   │   ├── Metadata/                # Metadatos de validación
│   │   │   ├── WorkerViewMetadata.cs
│   │   │   ├── CourseViewMetadata.cs
│   │   │   └── [Entidad]ViewMetadata.cs
│   │   │
│   │   └── Base/                    # Views base del framework
│   │       ├── AttachmentView.cs
│   │       ├── SecurityUserView.cs
│   │       └── Metadata/
│   │           └── AttachmentViewMetadata.cs
│   │
│   ├── PartialViews/                # Extensiones parciales de Views
│   │   └── WorkerView.Custom.cs     # Propiedades calculadas
│   │
│   └── Validations/                 # Validaciones personalizadas
│       └── CustomValidations.cs
│
├── [Proyecto].Data/                 # Capa de Acceso a Datos
│   ├── EntityModel.cs               # DbContext principal
│   ├── DataConsts.cs                # Constantes de datos
│   │
...
│   │   │
│   │   └── Base/                    # Repositorios base del framework
│   │       ├── AttachmentRepository.cs
│   │       └── SecurityUserRepository.cs
│   │
│   ├── DataModel/                   # Configuraciones EF Core
│   │   └── EntityConfigurations/
│   │       └── WorkerConfiguration.cs
│   │
│   └── CodeTemplates/               # Plantillas de scaffolding
│       └── EFCore/
│
├── [Proyecto].Services/             # Capa de Lógica de Negocio
│   ├── ServiceConsts.cs             # Constantes de servicios
│   ├── WorkerService.cs             # Servicios del dominio
│   ├── CourseService.cs
│   ├── ProjectService.cs
│   │
│   └── Base/                        # Servicios base del framework
│       ├── AttachmentService.cs
│       ├── AttachmentFileService.cs
│       ├── PermissionsService.cs
│       ├── SecurityUserService.cs
│       └── SecurityProfileService.cs
│
├── [Proyecto].Services.Tests/       # Tests de servicios
│   ├── TestInitialization.cs
│   ├── [Entidad]ServiceTests.cs
│   ├── Fixtures/
│   └── Mock/
│
├── [Proyecto].Data.Tests/           # Tests de repositorios
│   ├── TestInitialization.cs
│   ├── BaseRepositoryTest.cs
│   ├── appsettings.Test.json
│   └── Mock/
│
├── [Proyecto].HelixGenerator/       # Generador de código
│   └── (Herramienta de generación)
│
├── [Proyecto].HelixGenerator.Tests/ # Tests del generador
│
├── [Proyecto]Docker/                # Configuración Docker
│   └── docker-compose.yml
│
├── packages/                        # Paquetes NuGet (si legacy)
│
...
Helix6 aplica convenciones estrictas de nomenclatura para mantener consistencia y permitir el autodescubrimiento automático de componentes.

#### 2.2.1 Nomenclatura de Clases

| Tipo de Clase | Convención | Ejemplo |
|--------------|-----------|---------|
| **Entidad (DataModel)** | PascalCase, singular | `Worker`, `Course`, `Project` |
| **View (DTO)** | `[Entidad]View` | `WorkerView`, `CourseView` |
| **Metadata** | `[Entidad]ViewMetadata` | `WorkerViewMetadata` |
| **Servicio** | `[Entidad]Service` | `WorkerService`, `CourseService` |
| **Interfaz de Servicio** | `I[Entidad]Service` | `IWorkerService` |
| **Repositorio** | `[Entidad]Repository` | `WorkerRepository` |
| **Interfaz de Repositorio** | `I[Entidad]Repository` | `IWorkerRepository` |
| **Endpoints** | `[Entidad]Endpoints` | `WorkerEndpoints` |
| **Constantes** | `[Contexto]Consts` | `ApiConsts`, `ServiceConsts`, `DataConsts` |

#### 2.2.2 Nomenclatura de Propiedades y Campos
...
public class Worker : IEntityBase
{
    ...
    // Claves foráneas: [Entidad]Id
    public int WorkerTypeId { get; set; }
    
    // Propiedades de navegación: Nombre de la entidad
    public virtual WorkerType? WorkerType { get; set; }
    
    // Colecciones: Plural del nombre de entidad
    public virtual ICollection<Worker_Course>? Courses { get; set; }
    
    // Propiedades de auditoría (implementan IEntityBase)
    public int AuditCreationUser { get; set; }
    public DateTime? AuditCreationDate { get; set; }    // ✅ siempre DateTime?
    public int AuditModificationUser { get; set; }
    public DateTime? AuditModificationDate { get; set; } // ✅ siempre DateTime?
    public DateTime? AuditDeletionDate { get; set; }
}
```

#### 2.2.3 Nomenclatura de Variables y Parámetros
...
public class WorkerService : BaseService<WorkerView, Worker, WorkerViewMetadata>
{
    ...
    // Constructor: parámetros en camelCase
    ...
    // Métodos: parámetros en camelCase
...
#### 2.2.4 Nomenclatura de Métodos

```csharp
// Métodos públicos: PascalCase, verbos descriptivos
public async Task<WorkerView?> GetById(int id)
public async Task<List<WorkerView>> GetActiveWorkers()
public async Task<bool> ValidateWorkerCode(string code)

// Métodos privados: PascalCase (igual que públicos)
private async Task<string> GenerateWorkerCode()
private bool IsValidAge(DateTime birthDate)
private void LogOperation(string operation)

// Métodos de extensión: PascalCase, contexto claro
public static void AddDependencyInjection(this IServiceCollection services)
public static void MapWorkerEndpoints(this IEndpointRouteBuilder app)
```

#### 2.2.5 Nomenclatura de Interfaces

```csharp
// Interfaces: prefijo I + PascalCase
public interface IWorkerRepository : IBaseRepository<Worker>
public interface IWorkerService : IBaseService<WorkerView, Worker, WorkerViewMetadata>
public interface IUserContext
public interface IApplicationContext
public interface IAttachmentSource<TAttachment>
```

#### 2.2.6 Nomenclatura de Constantes
...
public static class ServiceConsts
{
    ...
    public static class Validations
    {
        ...
        {
            // Claves de error: UPPER_CASE con underscore
            public const string TRAINEE_WORKER_MUST_BE_FROM_AGENCY = "TRAINEE_WORKER_MUST_BE_FROM_AGENCY";
            public const string INVALID_BIRTH_DATE = "INVALID_BIRTH_DATE";
            public const string DUPLICATE_WORKER_CODE = "DUPLICATE_WORKER_CODE";
        }
    }
}
```

#### 2.2.7 Nomenclatura de Archivos

| Tipo de Archivo | Convención | Ejemplo |
|-----------------|-----------|---------|
| **Clase** | `[NombreClase].cs` | `Worker.cs`, `WorkerService.cs` |
| **Interfaz** | `I[NombreInterfaz].cs` | `IWorkerRepository.cs` |
| **Partial Class** | `[NombreClase].Custom.cs` | `WorkerView.Custom.cs` |
| **Constantes** | `[Contexto]Consts.cs` | `ApiConsts.cs`, `ServiceConsts.cs` |
| **Configuración** | `[Nombre]Configuration.cs` | `AuthConfiguration.cs` |
| **Extensiones** | `[Contexto]Extensions.cs` | `DependencyInjection.cs` |
| **Tests** | `[Clase]Tests.cs` | `WorkerServiceTests.cs` |

#### 2.2.8 Nomenclatura de Endpoints

```csharp
// URLs de endpoints: /api/[Entidad]/[Método]
/api/Worker/GetById
/api/Worker/Insert
/api/Worker/Update
/api/Worker/Delete
...
// Métodos de generación en EndpointHelper
...
#### 2.2.9 Nomenclatura de Relaciones en Base de Datos

```csharp
// Tablas intermedias (many-to-many): [Entidad1]_[Entidad2]
public class Worker_Course : IEntityBase
{
...
// Archivo: Worker_Course.cs  |  Tabla en BD: Worker_Course
```

#### 2.2.10 Nomenclatura de Configuración

```csharp
// appsettings.json - Secciones en PascalCase
{
  "ConnectionStrings": { ... },
  "HelixConfiguration": { ... },
  "Authentication": { ... },
...
// Binding a clases con el mismo nombre de sección
public class HelixConfiguration
public class AttachmentDriveSource
```

#### 2.2.11 Nomenclatura de Namespaces

```csharp
// Estructura: [NombreProyecto].[Capa].[Subcapa]
namespace Helix6.Back.Api
namespace Helix6.Back.Api.Extensions
namespace Helix6.Back.Api.Endpoints.Base.Generator
namespace Helix6.Back.Services
namespace Helix6.Back.Services.Base
namespace Helix6.Back.Data.Repository
namespace Helix6.Back.Data.Repository.Interfaces
namespace Helix6.Back.DataModel
namespace Helix6.Back.DataModel.Base
namespace Helix6.Back.Entities.Views
namespace Helix6.Back.Entities.Views.Metadata
```

#### 2.2.12 Convenciones Especiales

```csharp
// Nullable Reference Types - activado en todo el proyecto
#nullable enable

// Parámetros opcionales - sufijo con ?
public async Task<WorkerView?> GetById(int id, string? configurationName = null)

// Async/Await - preferir nombres sin sufijo Async en servicios/repositorios
public async Task<Worker?> GetById(int id)      // ✓ Correcto
public async Task<Worker?> GetByIdAsync(int id) // ✗ Redundante en este contexto

// Propiedades calculadas - no mapear a BD
[NotMapped]
...
1. **Un archivo por clase**: Cada clase, interfaz o enum debe estar en su propio archivo.
2. **Carpetas por funcionalidad**: Agrupar archivos relacionados en carpetas temáticas (`Repository`, `Services`, `Endpoints`).
3. **Separación generado/personalizado**:
   - Código generado en `Base/Generator/`
   - Código personalizado en raíz o `Custom/`
   - Extensiones parciales con sufijo `.Custom.cs`
4. **Imports organizados**:
   - System namespaces primero
   - Third-party packages después
   - Project namespaces al final
   - Ordenados alfabéticamente
5. **Nomenclatura consistente**: Seguir las convenciones sin excepciones para permitir autodescubrimiento.

---

## 3. Estructura de Capas y Proyectos

### 3.1 Helix6.Base (Librería Base)
...
### 3.2 Helix6.Base.Domain (Capa de Dominio Base)
...
### 3.3 Helix6.Base.Utils (Utilidades Base)
...
### 3.4 [Proyecto].Api (Capa de Presentación - Web API)
...
### 3.5 [Proyecto].DataModel (Capa de Modelo de Datos)
...
### 3.6 [Proyecto].Data (Capa de Acceso a Datos)
...
### 3.7 [Proyecto].Entities (Capa de Entidades/DTOs)
...
### 3.8 [Proyecto].Services (Capa de Lógica de Negocio)
...
### 3.9 [Proyecto].Services.Tests (Pruebas de Servicios)
...
### 3.10 [Proyecto].Data.Tests (Pruebas de Repositorios)
...
### 3.11 [Proyecto].HelixGenerator (Generador de Código)
...
## 4. Archivo HelixEntities.xml

### 4.1 Propósito y Función
...
### 4.3 Configuraciones Soportadas
...
### 5.1 Flujo de una Petición HTTP
...
### 5.3 Transacciones y Unidad de Trabajo
...
### 6.8 Otras Dependencias Importantes
...
## 8. Bootstrapping y Program.cs

El archivo `Program.cs` orquesta el inicio de la aplicación.

### 8.1 Secuencia de Configuración en Program.cs
...
## 9. Generación Automática de Código
...
### 9.2 Componentes Generados Automáticamente
...
### 9.3 Convenciones de Generación
...
### 10.3 EndpointAccess y Seguridad
...
### 11.5 Mapeo de Claims según Identity Server
...
### 12.1 Campos de Auditoría Estándar
...
### 12.2 Actualización Automática de Auditoría
...
### 12.3 Auditoría en Transacciones
...
## 13. Sistema de Archivos Adjuntos (Attachments)

### 13.1 Implementaciones de `IAttachmentSource`
...
## 14. Sistema de Versionado y Vigencia

### 14.1 Entidades con Versionado (`IsVersionEntity`)
...
### 14.2 Entidades con Vigencia (`IsValidityEntity`)
...
### 15.1 Configuración de Culturas
...
## 16. Integración con Frontend (Kendo UI)

### 16.1 Endpoint `GetAllKendoFilter`
...
### 16.2 `IGenericFilter` y `HelixFilterMapping`
...
## 19. Despliegue y Containerización
...
## 20. Patrones y Mejores Prácticas

### 20.1 Patrones Aplicados
...
### 20.3 Organización del Repositorio
...
## 21. Extensibilidad y Personalización
...
## 22. Lista Completa de Métodos del Framework Base

### 22.1 `BaseService<TView, TEntity, TMetadata>`
```csharp
Task<TView?> GetById(int id, string? configurationName = null);
Task<List<TView>> GetByIds(List<int> ids, string? configurationName = null);
Task<TView?> GetNewEntity();
...
## 23. Diagramas y Flujos (Descripción Textual)

### 23.1 Diagrama de Capas
```
┌─────────────────────────────────────────┐
...
## 24. Escenarios de Uso y Ejemplos

### 24.1 Crear una Nueva Entidad
...
### 24.2 Agregar Validación Personalizada
```csharp
...
### 24.3 Agregar Lógica Pre/Post Acción
```csharp

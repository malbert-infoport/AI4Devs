import { Injectable, OnDestroy, Renderer2, RendererFactory2, inject, DOCUMENT } from '@angular/core';
...
export class ThemeService implements OnDestroy {
  ...
  private static readonly LIGHT_MODE_CSS_CLASS_NAME: string = 'HLXLibrary-light-theme';
  private static readonly DARK_MODE_CSS_CLASS_NAME: string = 'HLXLibrary-dark-theme';
  ...
  private setMode(darkMode: boolean): void {
    if (darkMode) {
      ...
      this.renderer.addClass(this.document.body, ThemeService.DARK_MODE_CSS_CLASS_NAME);
      this.setKendoUiControlsMode('kendoui-dark.css');
    } else {
      this.renderer.removeClass(this.document.body, ThemeService.DARK_MODE_CSS_CLASS_NAME);
  ...
  apply(): void {
    /**
     * TODO: Deshabilitamos la funcionalidad de cambio de tema
     */
  ...
  private startSystemModeSynchronization(): void {
    if (!this.destructionSubject && this.document.defaultView) {
      ...
      fromEvent(this.document.defaultView.matchMedia(ThemeService.SYSTEM_DARK_MODE_MEDIA_QUERY), 'change')
